  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.listCard}>
          ...
          {checkItems.map((item, i) => (
            <TouchableOpacity key={i} style={styles.checkRow} onPress={() => toggleCheck(i)}>
              ...
              <Text style={[styles.checkText, checked[i] && { color: colors.gray900 }]}>{item}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <TouchableOpacity
          style={[styles.nextBtn, !allChecked && { opacity: 0.4 }]}
          onPress={() => allChecked && navigation.navigate('PreQuestionnaire')}
          disabled={!allChecked}
        >
          <Text style={styles.nextBtnText}>下一步：採樣前問卷 ›</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  iconArea: { alignItems: 'center', paddingVertical: 12, gap: 8, marginBottom: 20, marginTop: 60 },
  iconBox: {
  ...
  checkboxDone: { backgroundColor: colors.successLight, borderColor: colors.success },
  checkmark: { fontSize: 12, color: colors.success },
  checkText: { fontSize: typography.sizes.md, color: colors.gray400, flex: 1 },
